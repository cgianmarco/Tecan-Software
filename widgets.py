...
class Grid(QWidget):
    ...
    def update_background_color(self, start_row, end_row, start_col, end_col, color):
...
class Legend(QWidget):
    ...
    def add_item(self, color):
        color_box = QLineEdit()
        stylesheet = "border-style:solid; border-color:#aaa; border-width: 1px;background-color:" + color + ";"
        color_box.setStyleSheet(stylesheet)
        self.__layout.addWidget(color_box)
...
class Legend(QWidget):
    ...
    def clear(self):
        for i in reversed(range(self.__layout.count())): 
            if not isinstance(self.__layout.itemAt(i), QSpacerItem):
                self.__layout.itemAt(i).widget().deleteLater()
            else:
                self.__layout.takeAt(i)
    ...
    def add_item(self, color):
        ...
        # colorbox
        color_box = QLineEdit()
        stylesheet = "border-style:solid; border-color:#aaa; border-width: 1px;background-color:" + color.name() + ";"
        color_box.setStyleSheet(stylesheet)
        color_box.setDisabled(True)
        color_box.setFixedWidth(25)
        color_box.setFixedHeight(25)

        color_field = QWidget()
        self.__layout.addWidget(color_field)
        if color not in self.__legend.keys():
            self.__legend[color] = ""


        # colorname
        color_name = QLineEdit()
        color_name.setText(self.__legend[color])
        color_name.textChanged.connect(lambda text : self.on_color_text_changed(color, text))

        color_name.setFixedHeight(25)
        color_name.setFixedWidth(100)
        ...
        color_field.setLayout(layout)

    def on_color_text_changed(self, color, text):
        self.__legend[color] = text
